...
void MyDetectorConstruction::ConstructCherenkov()
{
    ...
    for(G4int i = 0; i < nRows; i++)
    {
        for(G4int j = 0; j < nCols; j++)
        {
            ...
            physDetector = new G4PVPlacement(0, G4ThreeVector(-1*xWorld+xWorld*(2*i+1)/nRows, -1*yWorld+yWorld*(2*j+1)/nCols, 0.49*m), logicDetector, "physDetector", logicWorld, false, j+i*nCols, true);
            physDetector2 = new G4PVPlacement(rotM, G4ThreeVector(0.51*m, -1*yWorld+yWorld*(2*j+1)/nCols, -1*zWorld+zWorld*(2*i+1)/nRows), logicDetector, "physDetector2", logicWorld, false, j+i*nCols, true);
